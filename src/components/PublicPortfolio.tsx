  ...
  return (
    <div className="page-shell">
      ...
      <main>
        ...
        <section id="about" className="section-frame about-section"><div className="about-intro"><div><p className="eyebrow">SOBRE MIM</p><h2>{settings.bio.split('\n').map((paragraph) => <span key={paragraph}>{paragraph}<br /><br /></span>)}</h2></div><div className="portrait-placeholder">{settings.photoUrl ? <img className="portrait-photo" src={settings.photoUrl} alt={settings.name} /> : <><div className="portrait-glow" /><span>FOTO</span></>}</div></div><div className="specialties-wrap">{['EDIÇÃO DE VÍDEO', 'MOTION DESIGN', 'REDES SOCIAIS', 'COMERCIAIS', 'ANÚNCIOS', 'COR E SOM'].map((item) => <span key={item} className="specialty-pill">{item}</span>)}</div></section>
